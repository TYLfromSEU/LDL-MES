...
def compute_matrix_C(D):
    ...
    row_sum=c_temp.sum(axis=1).reshape(c_temp.sum(axis=1).shape[0],1)  #每行的和
...
def obj_func1(w, x, d_, f_dim, l_dim):   #w, features, label_real, z, features_dim, labels_dim  
    ...
    for i in range(m):
        ...
        d_[i]=np.array(d_[i])
        term1+=np.sum(d_[i]*np.log((d_[i]+10**-6)/(P+10**-6)))   #kl散度
...
def gradient_cij(w,x,features_dim,labels_dim):
    ...
    disProb = modProb / (sumProb.reshape(-1, 1) + 0.000001)    #预测的P矩阵
    C=compute_matrix_C(disProb)    #C矩阵
...
def gradient_theta(w, x, d_,f_dim, l_dim):       #梯度函数
...
if __name__ == "__main__":
    data1 = read_mat(r"datasets\Natural_Scene.mat")   #data1:dict
    ...
    for t in range(1):
        ...
        for i in range(len(x_train)):   # 后期len(features)改为训练集大小
            x_result[kmeans_result[i]].append(list(features[i]))          #D(i)实例的特征   S×243
            d_result[kmeans_result[i]].append(list(label_real[i]))         #D(i)真实分布
        ...
        result = fmin_l_bfgs_b(obj_func1, w, gradient_theta, args=(x_result, d_result,features_dim, labels_dim),
                                   pgtol=1e-4, maxiter=10000)      #取最小值时的w
        w = result[0]
